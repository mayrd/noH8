...
/**
 * TikTok DOM selector strategy.
 *
 * TikTok renders each comment as a `<p data-e2e="comment-level-1">` whose own
 * text content IS the comment body (nested replies use `comment-level-2`). The
 * author handle lives in a neighbouring link/span tagged with `data-e2e`
 * attributes. We rely on these stable e2e markers rather than hashed classes.
 */
const COMMENT_PRIMARY_SELECTORS = ['p[data-e2e="comment-level-1"]'];
const COMMENT_SECONDARY_SELECTORS = [
  'p[data-e2e="comment-level-2"]',
  'p[data-e2e="comment-level-3"]',
];

/** Fallback selector used to grab the comment body text. */
const COMMENT_TEXT_SELECTOR = 'span[data-e2e="comment-text"], span[data-e2e="comment-content"]';
...
export default class TikTokAdapter extends BaseAdapter {
  ...
  private resolveId(item: ElementLike, author: string, text: string): string {
    const attr =
      ...
      item.getAttribute?.('data-e2e') ||
      item.getAttribute?.('id');
    if (attr) return attr;
    return `tiktok-${this.hash(author + text)}`;
  }

  private parseComment(item: ElementLike): CommentData | null {
        // The container is itself the body text node; fall back to a child node.
  ...
  /** Parses the TikTok DOM for top-level comments. */
  extractComments(): CommentData[] {
    if (!this.root) return [];

    const items = selectCommentContainers(this.root, TikTokAdapter.selectors);
  ...
  /** Watches the DOM for lazy-loaded comment sections. */
